...
/*void desenha_fundo(int diff){
    ...
    if(diff == 3) DrawTextureRec(FundoLoverTxt, (Rectangle){0,0,1200,800}, (Vector2){0,0}, RAYWHITE);
            ...
            case GAME:
            {
                ...
                for(int i=0;i<N_BARRA;i++){
                    ...
                    if(bad_blood){
                        for(int j=0;j<10; j++){
                            shot[0].posit.x += 5;
                            if(shot[j-1].posit.x > 1000){
                                shot[j].posit.x += 5;
                                shot[j].posit.y += GetRandomValue(-5,6);
                            }
                            if(CheckCollisionCircleRec(shot[j].posit, shot[j].radius,tubes[i].tubeUp.rec) || CheckCollisionCircleRec(shot[j].posit, shot[j].radius, tubes[i].tubeDown.rec)){
                                if(CheckCollisionCircleRec(shot[j].posit, shot[j].radius,tubes[i].tubeUp.rec)){
                                    tubes[i].tubeUp.rec.y = -801;
                                }
                                if(CheckCollisionCircleRec(shot[j].posit, shot[j].radius, tubes[i].tubeDown.rec)){
                                     tubes[i].tubeDown.rec.y = 801;
                                }
                            }
                        }
                    ...
                    if(powerUp.mode){
                        if(score % POWER_UP_SPACE == 15) {
                            if(pup_spwn == 0){
                                Vector2 teste = {389*5, GetRandomValue(150, 650)};
                                powerUp.posit = teste;
                                powerUp.radius = 40;
                                pup_spwn == 1;
    ...
    BeginDrawing();
        ...
        switch(currentScreen){
            ...
            case GAME:
            {
                ...
                if(powerUp.mode){
                    DrawCircleV(powerUp.posit, powerUp.radius, GREEN);
                ...
                DrawText(itoa(powerUp.type,test5,5), screenWidth - 600, 20, 30, GREEN);
