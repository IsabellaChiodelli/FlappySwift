...
void le_rank(char* rank1_nome, char* rank1_ponto, char* rank2_nome, char* rank2_ponto,char* rank3_nome, char* rank3_ponto,char* rank4_nome, char* rank4_ponto,char* rank5_nome, char* rank5_ponto, char* out, int *p1, int *p2, int *p3, int *p4, int *p5){
    FILE *ptr;
...
void desenha_tubos(int diff,Texture2D tu1, Texture2D td1, Texture2D tu2, Texture2D td2, Texture2D tu3, Texture2D td3, Texture2D t4, Texture2D t5, Tubes* tubes){
    int i =0;
    while(i<N_BARRA){
        if(diff == 1) {
            DrawTextureRec(tu1, (Rectangle){0,0,tubes[i].tubeUp.rec.width,tubes[i].tubeUp.rec.height}, (Vector2){tubes[i].tubeUp.rec.x, tubes[i].tubeUp.rec.y}, RAYWHITE);
            DrawTextureRec(td1, (Rectangle){0,0,tubes[i].tubeDown.rec.width,tubes[i].tubeDown.rec.height}, (Vector2){tubes[i].tubeDown.rec.x, tubes[i].tubeDown.rec.y}, RAYWHITE);
        }
        if(diff == 2) {
            DrawTextureRec(tu2, (Rectangle){0,0,tubes[i].tubeUp.rec.width,tubes[i].tubeUp.rec.height}, (Vector2){tubes[i].tubeUp.rec.x, tubes[i].tubeUp.rec.y}, RAYWHITE);
            DrawTextureRec(td2, (Rectangle){0,0,tubes[i].tubeDown.rec.width,tubes[i].tubeDown.rec.height}, (Vector2){tubes[i].tubeDown.rec.x, tubes[i].tubeDown.rec.y}, RAYWHITE);
        }
        if(diff == 3){
            DrawTextureRec(tu3, (Rectangle){0,0,tubes[i].tubeUp.rec.width,tubes[i].tubeUp.rec.height}, (Vector2){tubes[i].tubeUp.rec.x, tubes[i].tubeUp.rec.y}, RAYWHITE);
            DrawTextureRec(td3, (Rectangle){0,0,tubes[i].tubeDown.rec.width,tubes[i].tubeDown.rec.height}, (Vector2){tubes[i].tubeDown.rec.x, tubes[i].tubeDown.rec.y}, RAYWHITE);
        }
        if(diff == 4){
            DrawTextureRec(t4, (Rectangle){0,0,tubes[i].tubeUp.rec.width,tubes[i].tubeUp.rec.height}, (Vector2){tubes[i].tubeUp.rec.x, tubes[i].tubeUp.rec.y}, RAYWHITE);
            DrawTextureRec(t4, (Rectangle){0,0,tubes[i].tubeDown.rec.width,tubes[i].tubeDown.rec.height}, (Vector2){tubes[i].tubeDown.rec.x, tubes[i].tubeDown.rec.y}, RAYWHITE);
        }
        if(diff == 5){
            DrawTextureRec(t5, (Rectangle){0,0,tubes[i].tubeUp.rec.width,tubes[i].tubeUp.rec.height}, (Vector2){tubes[i].tubeUp.rec.x, tubes[i].tubeUp.rec.y}, RAYWHITE);
            DrawTextureRec(t5, (Rectangle){0,0,tubes[i].tubeDown.rec.width,tubes[i].tubeDown.rec.height}, (Vector2){tubes[i].tubeDown.rec.x, tubes[i].tubeDown.rec.y}, RAYWHITE);
            }
        i++;
        }
    i=0;
...
void desenha_borda(int diff, Texture2D BordaTx){
    if(diff == 1) DrawTextureRec(BordaTx, (Rectangle){0,0,510,110}, (Vector2){SCREEN_WIDTH/2 - 255, SCREEN_HEIGHT/2 - 230}, RAYWHITE);
    if(diff == 2) DrawTextureRec(BordaTx, (Rectangle){0,0,510, 110}, (Vector2){SCREEN_WIDTH/2 - 255,SCREEN_HEIGHT/2 - 120}, RAYWHITE);
    if(diff == 3) DrawTextureRec(BordaTx, (Rectangle){0,0,510, 110}, (Vector2){SCREEN_WIDTH/2 - 255,SCREEN_HEIGHT/2 - 10}, RAYWHITE);
    if(diff == 4) DrawTextureRec(BordaTx, (Rectangle){0,0,510, 110}, (Vector2){SCREEN_WIDTH/2 - 255,SCREEN_HEIGHT/2 + 100}, RAYWHITE);
    if(diff == 5) DrawTextureRec(BordaTx, (Rectangle){0,0,510, 110}, (Vector2){SCREEN_WIDTH/2 - 255,SCREEN_HEIGHT/2 + 210}, RAYWHITE);
}
...
int main(void){
    ...
    char backspc = '\n';
    FILE *write_ptr;
    le_rank(rank1_nome, rank1_ponto, rank2_nome, rank2_ponto, rank3_nome, rank3_ponto, rank4_nome, rank4_ponto, rank5_nome, rank5_ponto, out, &p1, &p2, &p3, &p4, &p5);
    ...
    Texture2D TaylorGolfeTx = LoadTexture("resources/TaylorGolfe.png");
    ...
    Texture2D FearlessBarTx = LoadTexture("resources/BarFearless.png");
    Texture2D RepBarTx = LoadTexture("resources/RepBar.png");
            ...
            case SCORE:
            {
                ...
                if (CheckCollisionPointRec(mousePoint, btnDone)){
                    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)){
                        ...
                        //Abre o documento atualizado e atualiza os parametros
                        le_rank(rank1_nome, rank1_ponto, rank2_nome, rank2_ponto, rank3_nome, rank3_ponto, rank4_nome, rank4_ponto, rank5_nome, rank5_ponto, out, &p1, &p2, &p3, &p4, &p5);
                        //Zera o nome
                        name[0] = '\0';
                        letterCount = 0;
    ...
    BeginDrawing();
        ...
        switch(currentScreen){
            ...
            case START:{
                ...
                desenha_tubos(diff,UpBarFolkTx, DownBarFolkTx, UpBar1989Tx, DownBar1989Tx, UpBarLoverTx, DownBarLoverTx, FearlessBarTx, RepBarTx, tubes);
                ClearBackground(RAYWHITE);
                //Desenha o Aviso para iniciar o jogo
                DrawText("Press Space", 480, 380, 30, RED);
            ...
            case GAME:
            {
                ...
                else if(golfe) DrawTextureRec(TaylorGolfeTx, (Rectangle){0,0,player.rec.width+63,player.rec.height+3}, (Vector2){player.rec.x-45, player.rec.y-3}, RAYWHITE);
            ...
            case DIFF:
            {
                ...
                DrawTextureRec(Diff5Tx, (Rectangle){0,0,btn5Diff.width,btn5Diff.height}, (Vector2){btn5Diff.x, btn5Diff.y}, RAYWHITE);
                //Desenha a barra dependendo da seleção
                desenha_borda(diff, BordaTx);
