...
typedef struct TexturePack {
    ...
    Texture2D BateuAsaTx;
    Texture2D BateuAsaGolfeTx;
...
void le_dificuldade(void){
    ...
    fscanf(fptr, "%d %d %d %d %d %d %d\n",&score_threshold,&gap,&inc_gap,&dif_max_altura,&inc_dif_max_altura,&vel_obstaculos,&inc_vel_obstaculos);
    if(score_threshold < N_BARRA + 1) score_threshold = N_BARRA + 1;
...
void desenha_jogo(void){
    ...
    if(invisible_string){
        if (grav>40) {
           DrawTextureRec(texturas.BateuAsaTx, (Rectangle){0,0,player.rec.width+63,player.rec.height+3}, (Vector2){player.rec.x-45, player.rec.y-3},  CLITERAL(Color){ 255, 255, 255, 128 });
        }
        else DrawTextureRec(texturas.TaylorTx, (Rectangle){0,0,player.rec.width+63,player.rec.height+3}, (Vector2){player.rec.x-45, player.rec.y-3},  CLITERAL(Color){ 255, 255, 255, 128 });
    }
    else if(golfe){
        if (grav>40) {
           DrawTextureRec(texturas.BateuAsaGolfeTx, (Rectangle){0,0,player.rec.width+63,player.rec.height+3}, (Vector2){player.rec.x-45, player.rec.y-3},  RAYWHITE);
        }
        else DrawTextureRec(texturas.TaylorGolfeTx, (Rectangle){0,0,player.rec.width+63,player.rec.height+3}, (Vector2){player.rec.x-45, player.rec.y-3}, RAYWHITE);
    }
    else{
        if (grav>40) {
           DrawTextureRec(texturas.BateuAsaTx, (Rectangle){0,0,player.rec.width+63,player.rec.height+3}, (Vector2){player.rec.x-45, player.rec.y-3},  RAYWHITE);
        }
...
void init_program(void){
    ...
    texturas.BateuAsaTx = LoadTexture("resources/BateuAsa.png");
    texturas.BateuAsaGolfeTx = LoadTexture("resources/BateuAsaGolfe.png");
